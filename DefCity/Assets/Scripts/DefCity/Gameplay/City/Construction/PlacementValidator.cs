...
namespace DefCity.Gameplay.City.Construction
{
    public class PlacementValidator : MonoBehaviour
    {
        ...
        public bool CanPlace(GameObject prefab, Vector3 position, Quaternion rotation, out string failureReason)
        {
            if (!TryGetPlacementBounds(prefab, position, rotation, out Bounds placementBounds, out failureReason))
            {
                return false;
            }
            ...
            out string failureReason)
        {
            ...
            Collider[] colliders = prefab.GetComponentsInChildren<Collider>(false);
            Matrix4x4 targetRootMatrix = Matrix4x4.TRS(position, rotation, prefab.transform.localScale);
