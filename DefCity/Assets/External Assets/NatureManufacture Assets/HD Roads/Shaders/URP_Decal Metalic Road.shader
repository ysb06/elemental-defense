Shader "NatureManufacture Shaders/URP Decal Metalic Road "
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			half _MainRoadMetalicPower;
			half _MainRoadSmoothnessPower;
			half _MainRoadAmbientOcclusionPower;
			float _AlphaClip;
			float _Cutoff;
						 ...
						 ) : SV_Target
			{
				...
				float3 BaseColor = lerpResult480.rgb;
				float3 Normal = lerpResult479;
				float3 Specular = 0.5;
				float Metallic = ( tex2DNode2.r * _MainRoadMetalicPower );
				float Smoothness = ( tex2DNode2.a * _MainRoadSmoothnessPower );
				float Occlusion = clampResult96;
				float3 Emission = 0;
