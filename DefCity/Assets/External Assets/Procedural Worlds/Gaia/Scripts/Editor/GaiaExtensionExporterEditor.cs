...
namespace Gaia
{
    ...
    public class GaiaExtensionExporterEditor : EditorWindow
    {
        ...
        private void SaveGX()
        {
            ...
            Template template;
            string hdrPath = PWCommon5.AssetUtils.GetAssetPath("GaiaXtnHeader", "txt");
            string ftrPath = PWCommon5.AssetUtils.GetAssetPath("GaiaXtnFooter", "txt");
        ...
        private string DumpObject(object src, string name, int tabDepth, bool prefixType = true, bool instantiate = true)
        {
            ...
            foreach (FieldInfo f in fields)
            {
                ...
                else if (f.FieldType == typeof(UnityEngine.Texture2D))
                {
                    ...
                    {
                        retS.AppendLine(string.Format("{0}{1}.{2} = GetTexture2D(\"{3}\");", tabs, name, f.Name, AssetDatabase.GetAssetPath((UnityEngine.Texture2D)f.GetValue(src))));
                    }
                    else
                    {
                ...
                else if (f.FieldType == typeof(UnityEngine.GameObject))
                {
                    ...
                    {
                        retS.AppendLine(string.Format("{0}{1}.{2} = GetGameObject(\"{3}\");", tabs, name, f.Name, AssetDatabase.GetAssetPath((UnityEngine.GameObject)f.GetValue(src))));
                    }
                    else
                    {
                        retS.AppendLine(string.Format("{0}{1}.{2} = null;", tabs, name, f.Name));
                ...
                else if (f.FieldType == typeof(Gaia.SpawnRuleExtension[]))
                {
                    ...
                    {
                        retS.AppendLine(string.Format("{0}\t{1}.{2}[{3}] = GetSpawnRuleExtension(\"{4}\");", tabs, name, f.Name, idx, AssetDatabase.GetAssetPath((Gaia.SpawnRuleExtension)((System.Array)f.GetValue(src)).GetValue(idx))));
                    }
                }
