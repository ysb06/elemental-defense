...
namespace Gaia
{
    ...
    public class ScreenShotter : MonoBehaviour
    {
        ...
        {
            if (IsScreenshotHotkeyPressedThisFrame() || m_takeShot)
            {
                TakeScreenshotAndSave();
            }
        }

        public bool IsScreenshotHotkeyPressedThisFrame()
        {
#if ENABLE_INPUT_SYSTEM
            if (Keyboard.current != null)
            {
                KeyControl keyControl = GetKeyControlFromKeyCode(Keyboard.current, m_screenShotKey);
            ...
            return Input.GetKeyDown(m_screenShotKey);
#else
            return false;
#endif
        }
...
#if ENABLE_INPUT_SYSTEM
        private static KeyControl GetKeyControlFromKeyCode(Keyboard keyboard, KeyCode keyCode)
        {
            switch (keyCode)
            {
                case KeyCode.Return:
