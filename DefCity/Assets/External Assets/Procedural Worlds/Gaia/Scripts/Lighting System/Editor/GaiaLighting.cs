    ...
    public static class GaiaLighting
    {
                ...
                if (lightingProfile.m_lightSystemMode == GaiaConstants.GlobalSystemMode.Gaia)
                {
                    if (profile.m_profileType == GaiaConstants.GaiaLightingProfileType.ProceduralWorldsSky)
                    {
                        ...
                        if (renderPipeline != GaiaConstants.EnvironmentRenderer.HighDefinition)
                        {
                            ...
                            PW_VFX_Clouds clouds = GaiaUtils.FindOOT<PW_VFX_Clouds>();
                            if (clouds != null)
                            {
        ...
        public static void SetPostProcessingStatus(bool enabled)
        {
            try
            {
                ...
                {
                    PostProcessLayer layer = GaiaUtils.FindOOT<PostProcessLayer>();
                    if (layer != null)
                    {
                        if (enabled)
                        {
        ...
        {
#if UNITY_6000_0_OR_NEWER
            // Unity 6 removed Auto Generate lighting, so there is no workflow mode to toggle here.
            return;
#else
            if (enabled)
            {
        ...
        {
#if GENA_PRO
            GaiaTimeOfDayLightSync[] lightSyncs = GaiaUtils.FindOOTs<GaiaTimeOfDayLightSync>();
        ...
        {
#if UNITY_POST_PROCESSING_STACK_V2
            m_processLayer = GaiaUtils.FindOOT<PostProcessLayer>();
            if (m_processLayer != null)
            {
