	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				if (input.ase_color.a < _Threshold) {
					temp_output_22_0 = (input.ase_color.a / _Threshold);
				}


				float4 lerpResult16 = lerp( ( tex2DNode1 * _Color0 ) , ( tex2D( _Albedo, uv_Albedo ) * _Color ) , temp_output_22_0);
				
				float2 texCoord39 = input.ase_texcoord9.xy * float2( 1,1 ) + float2( 0,0 );
				...
				float2 uv_MetallicGlossMap2 = input.ase_texcoord9.xy * _MetallicGlossMap2_ST.xy + _MetallicGlossMap2_ST.zw;
		...
		Pass
		{
						 ...
						 )
			{
				...
				float2 texCoord39 = input.ase_texcoord5.xy * float2( 1,1 ) + float2( 0,0 );
				float3 unpack5 = UnpackNormalScale( tex2D( _BumpMap, texCoord39 ), _BumpScale1 );
				unpack5.z = lerp( 1, unpack5.z, saturate(_BumpScale1) );
