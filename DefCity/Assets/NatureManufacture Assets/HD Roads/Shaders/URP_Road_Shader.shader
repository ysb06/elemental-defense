Shader "NatureManufacture Shaders/URP Road Material"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
						 ...
						 ) : SV_Target
			{
				...
				float2 uv_DetailAlbedoMap = input.ase_texcoord7.xy * _DetailAlbedoMap_ST.xy + _DetailAlbedoMap_ST.zw;
				float4 tex2DNode486 = tex2D( _DetailAlbedoMap, uv_DetailAlbedoMap );
				...
				float4 tex2DNode537 = tex2D( _TextureSample3, uv_TextureSample3 );
				...
				float3 unpack4 = UnpackNormalScale( tex2D( _BumpMap, uv_MainTex ), _BumpScale );
				unpack4.z = lerp( 1, unpack4.z, saturate(_BumpScale) );
				float3 tex2DNode4 = unpack4;
				float3 unpack485 = UnpackNormalScale( tex2D( _DetailNormalMap, uv_DetailAlbedoMap ), _DetailNormalMapScale );
				unpack485.z = lerp( 1, unpack485.z, saturate(_DetailNormalMapScale) );
				float3 lerpResult479 = lerp( tex2DNode4 , BlendNormal( tex2DNode4 , unpack485 ) , tex2DNode481.a);
				float3 unpack535 = UnpackNormalScale( tex2D( _SecondRoadNormal, uv_TextureSample3 ), _SecondRoadNormalScale );
				...
				float3 unpack624 = UnpackNormalScale( tex2D( _DetailNormalMap, uv_DetailAlbedoMap ), _Float1 );
				unpack624.z = lerp( 1, unpack624.z, saturate(_Float1) );
				float3 lerpResult617 = lerp( tex2DNode535 , BlendNormal( lerpResult570 , unpack624 ) , tex2DNode481.a);
				float3 lerpResult593 = lerp( lerpResult479 , lerpResult617 , ( 1.0 - clampResult672 ).x);
				
				float4 tex2DNode2 = tex2D( _MetalicRAmbientOcclusionGHeightBEmissionA, uv_MainTex );
				float4 tex2DNode536 = tex2D( _SecondRoadMetallicRAmbientocclusionGHeightBSmoothnessA, uv_TextureSample3 );
				...
				float3 Normal = lerpResult593;
				float3 Specular = 0.5;
				float Metallic = lerpResult601;
				float Smoothness = lerpResult594;
				float Occlusion = lerpResult602;
				float3 Emission = 0;
				float Alpha = lerpResult628;
